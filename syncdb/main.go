...
func main() {
    ...
    for i<10000 {
		...
		if typ == "movie" {
			...
			if err == nil {
				if len(movie.ID) == 0 {
                    fmt.Println(strconv.Itoa(i) + " :" +  id + " has no info.")
                }
				if len(movie.ID) > 0 {
					fmt.Println(strconv.Itoa(i) + " :" + movie.ID + " " + movie.Title)
		...
		if typ == "tv" {
			...
			if err == nil {
				...
				if len(tv.ID) > 0 {
                    fmt.Println(strconv.Itoa(i) + " :" + tv.ID + " " + tv.Title)
